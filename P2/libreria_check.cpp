...
int main(int argc, const char **argv)
{
  ...
  if(c1.findClass({"Articulo", "ArticuloAlmacenable", "Autor",
    ...
    c1.functionWithReferencedMethod({"operator<<"},{{
	  "class std::basic_ostream<char> &","const class Articulo &"}},
      {"impresion_generica"},{{"class std::basic_ostream<char> &","_Bool"}},
      "Articulo",{"const"},
      "¿No llamas a impresion_generica en el operador de inserción?");
  ...
  if(c4.findClass({"Pedido"})) {
    ...
    */
    c4.listInitializerConstructor("Pedido", {"?"}, {"int", "class Fecha", "double"}, 
