...
int main()
{
  ...
  try {		 // O398 en vez de 0398 ;-) (era una O, no un cero)
    Tarjeta falsa(Numero("4539 4512 O398 7356"), lucas, Fecha(31, 12, Fecha().anno() + 3));
  }
  catch(Numero::Incorrecto& e) {
    numero_incorrecto(e);
  }
  try {
    Tarjeta judas(Numero("1234567890123456"), lucas, Fecha(31, 12, Fecha().anno() + 3));
  }
  catch(Numero::Incorrecto& e) {
    numero_incorrecto(e);
  }
  try {
    Tarjeta mala(Numero("123"), lucas, Fecha(31, 12, Fecha().anno() + 4));
  }
  catch(Numero::Incorrecto& e) {
    numero_incorrecto(e);
  }
  try {
    Tarjeta caduca(Numero("4222222222222"), lucas, Fecha(31, 12, Fecha().anno() - 1));
  }
...
//  krispin.compra(cppref);
  ...
  Pedido p2(usuario_ped, pedido_art, yoshua, american_express.numero(), 
	    --Fecha());	// pedido ayer
  Pedido p3(usuario_ped, pedido_art, krispin, mastercard.numero(), Fecha(5, 4, Fecha().anno()-1));
#endif
...
//  yessi.compra(adenda, 0); // no hace nada, adenda no está en el carro
...
#ifndef P2
  try {
    Pedido p4(usuario_ped, pedido_art, yessi, jcb.numero(), Fecha(5, 4, Fecha().anno()-1));
  }
  catch(Pedido::Impostor) {
    cout << "Yessi ha intentado pagar con una tarjeta que ya no tiene." << endl;
  }
  Pedido p4(usuario_ped, pedido_art, yessi, dinners_club.numero(), Fecha(5, 4, Fecha().anno()-1));
  cout << AZUL "\n---------------------PEDIDOS-----------------------\n"
  ...
  Pedido p7(usuario_ped, pedido_art, krispin, australian_bank_card.numero(), Fecha(5, 4, Fecha().anno()-1));
  cout << VERDE "*** Yoshua compra 2 artículos";
  yoshua.compra(enrique_alfarero, 2); 
  yoshua.compra(manualidades1);
  cout << " y hace su pedido.\n" NORMAL;
  Pedido p8(usuario_ped, pedido_art, yoshua, american_express.numero(), Fecha(5, 4, Fecha().anno()-1));
